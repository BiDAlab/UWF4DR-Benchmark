...
def _freq_from_path_py(path_tensor):
    ...
    _, buffer = cv2.imencode(".jpg", cv2.cvtColor(mag, cv2.COLOR_BGR2RGB)
...
def main():
    ...
    # IMPORTANT: do not touch the spatial path (it already matches the paper)
    if args.domain == "frequency":
        model = load_retfound_model_frequency(args.model, num_classes=args.num_classes)
    else:
